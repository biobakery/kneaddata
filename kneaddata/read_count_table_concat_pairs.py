...
def main():
    ...
    for file in contam_fastq:
        ...
        database_name = os.path.basename(file).split("_")[1]
        pair1, pair2, orphan1, orphan2 = count_paired_orphan(file)
        tag = "decontaminated "+database_name
        reads[sample_name][tag+" pair1"]=pair1
        reads[sample_name][tag+" pair2"]=pair2
        reads[sample_name][tag+" orphan1"]=orphan1
        reads[sample_name][tag+" orphan2"]=orphan2   
