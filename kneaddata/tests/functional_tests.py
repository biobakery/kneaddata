...
class TestFunctionalKneadData(unittest.TestCase):
    ...
    def test_trimmomatic_bowtie2_two_databases_paired_end_serial_discordant(self):
        ...
        bowtie2 database provided (both with the same name)
        Test running in serial alignment mode
        Test running with discordant alignment
        """
        ...
        command = ["kneaddata","--input",cfg.fastq_file,"--input",cfg.fastq_file,
                   "--output",tempdir,"--reference-db",cfg.bowtie2_db_folder,
                   "--reference-db",cfg.bowtie2_db_folder,"--serial"]
        utils.run_kneaddata(command)
    ...
    def test_trimmomatic_bowtie2_two_databases_paired_end_serial_discordant_concat(self):
        ...
        command = ["kneaddata","--input",cfg.fastq_file,"--input",cfg.fastq_file,
                   "--output",tempdir,"--reference-db",cfg.bowtie2_db_folder,
                   "--reference-db",cfg.bowtie2_db_folder,"--serial","--cat-pairs"]
        utils.run_kneaddata(command)
        ...
        # check the min number of main output files are expected
    ...
    def test_trimmomatic_bowtie2_paired_end_remove_intermedite_temp_output_discordant_cat_pairs_trf(self):
        ...
        Test running with remove intermediate temp output files
        Test with discordant alignments
        Test with pairs running as single end
        Test with TRF
        """
        ...
        command = ["kneaddata","--input",cfg.fastq_file,"--input",cfg.fastq_pair_file,
                   "--output",tempdir,"--reference-db",cfg.bowtie2_db_folder,
                   "--reference-db",cfg.bowtie2_db_folder, "--run-trf", "--cat-pairs"]
        utils.run_kneaddata(command)
