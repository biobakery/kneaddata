...
aSensitivity = np.divide(aData[:,8] + aData[:,9] + aData[:,10], aData[:,5] + aData[:,6] + aData[:,7])
print(aSensitivity)
aSensitivity[0] = 1
print(aSensitivity)
...
aSpecificity = np.divide(np.subtract(aNonBacterialOrig, aNonBacterialRemoved),aNonBacterialOrig )
print(aSpecificity)
...
plt.plot(aPercentContam, aSensitivity, 'o', color='r', label="P(Human|Human)")
plt.plot(aPercentContam, aSpecificity, '^', color='b', label="P(Nonhuman|Nonhuman)")
